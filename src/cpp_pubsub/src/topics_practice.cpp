...
public:
    TopicsPractice()
    : Node("topics_practice"), countdown_(5), in_spiral_mode_(true), linear_velocity_(0.9), angular_velocity_(1.1), current_x_(0.0), current_y_(0.0)
    {
        publisher_ = this->create_publisher<geometry_msgs::msg::Twist>("turtle1/cmd_vel", 10);
        pose_subscriber_ = this->create_subscription<turtlesim::msg::Pose>(
            "turtle1/pose", 10, std::bind(&TopicsPractice::pose_callback, this, std::placeholders::_1));
        timer_ = this->create_wall_timer(
            1s, std::bind(&TopicsPractice::timer_callback, this));
    }
...
int main(int argc, char * argv[])
{
    ...
    auto topics_practice = std::make_shared<TopicsPractice>();
    rclcpp::executors::MultiThreadedExecutor executor;
    executor.add_node(topics_practice);
    executor.spin();
    rclcpp::shutdown();
    return 0;
}